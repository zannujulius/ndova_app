...
async function recordStatusChange(
  ...
  changedById: string,
  extras: { adminNote?: string | null; providerId?: string | null; note?: string | null } = {}
) {
  const previousStatus = appointment.status;

  await sequelize.transaction(async (t) => {
    const updatePayload: Record<string, unknown> = { status: newStatus };
    if (extras.adminNote !== undefined) updatePayload.adminNote = extras.adminNote;
    if (extras.providerId !== undefined) updatePayload.providerId = extras.providerId;
...
export async function approveAppointment(
  ...
  await recordStatusChange(appointment, AppointmentStatus.APPROVED, userId, {
    ...
    providerId: input.providerId !== undefined ? input.providerId : appointment.providerId,
  });
  ...
  if (!roles.includes('ADMIN')) {
    if (roles.includes('PROVIDER')) {
      if (appointment.providerId !== userId) {
        throw ApiError.forbidden('You can only cancel appointments assigned to you');
      }
      ...
      throw ApiError.forbidden('You do not have permission to cancel this appointment');
    }
  }
